...
```text
tests/
  test_decoder.py       unit: decode each sentinel-cbc fixture .bin file
  test_encoder.py       unit: encode each reply template, decode result
  test_rules.py         unit: rule matching logic, count limiting, delay
  test_store.py         unit: SQLite store operations (uses :memory:)
  test_api.py           integration: FastAPI TestClient, no real SCTP
  test_sctp_transport.py  SCTP smoke test, @pytest.mark.sctp, Linux only
  validate_fixtures.py  CLI: cross-validate against sentinel-cbc fixture corpus
```

SCTP-dependent tests are marked `@pytest.mark.sctp` and skipped automatically
when pysctp is unavailable. All other tests run on Windows, macOS, and Linux.
