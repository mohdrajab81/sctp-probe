...
6. `47_swr_timeout_then_retry_success`
SentinelCBC first creates a repeat-until-stopped warning successfully. It then sends a stop request, the first stop attempt gets no response, and a later retry succeeds. Expected effect: the stop dispatch is retried once and then completes successfully.

7. `48_swr_timeout_exhausted_terminal`
SentinelCBC first creates a repeat-until-stopped warning successfully. It then sends a stop request and the simulator never replies. The stop is configured with only one allowed attempt. Expected effect: SentinelCBC stops retrying the stop operation and marks it as a terminal timeout.

8. `49_swr_transport_failure_exhausted_terminal`
SentinelCBC first creates a repeat-until-stopped warning successfully. It then sends a stop request, the first stop attempt gets no response, and the SCTP peer is unavailable when the retry is attempted. Expected effect: SentinelCBC exhausts the allowed stop attempts and finishes the stop dispatch as a terminal transport failure.
